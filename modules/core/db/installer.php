...
class Db_Installer {
    ...
    public static function install($model)
    {
        ...
        else
        {
            ...
            // If tableHasAndBelongsToMany, create key table
            foreach($model->tableHasAndBelongsToMany as $refModel)
            {
    ...
    private static function _createTable($model)
    {
        ...
        // Add any tableBelongsTo fields
        foreach($model->tableBelongsTo as $m)
        {
            $bits = explode('.', $m); // module.model
            $key = $bits[1] . '_id';

            // Foreign keys should be indexed
            array_push($model->tableIndexes, $key);
        ...
        */
        ...
        // Add indexes, if any
        foreach($model->tableIndexes as $k)
            $sql .= 'INDEX(' . $k . '),';

        // Add fulltext, if any
        if($model->tableFulltext)
            $sql .= sprintf('FULLTEXT(%s),', implode(',', $model->tableFulltext));
    ...
    {
        //$tableFields = Db::table($table)->describe();
        $fields = $model->tableFields;
        $tableFields = $model->describe();
