...
class Db_Orm extends Db_Query {
    ...
    public $tableHasOne = array();
    public $tableHasMany = array();
    public $tableBelongsTo = array();
    public $tableHasAndBelongsToMany = array();

    public $tableTimestamps = false;
    public $tableIndexes = array();
    public $tableFulltext = array();

    // Combine tableHasMany and tableHasOne because their the same thing
    public function __construct($table = null)
    {
        $this->tableHasMany = array_merge($this->tableHasMany, $this->tableHasOne);
    ...
    {
        /*
        $bits = explode('_', get_called_class());
        $module = $bits[0];
        $class = sprintf('%s_%sModel', $module, ucfirst($name));
        */

        /*
        // If tableHasOne or tableHasMany
        // Select via where using foreign key
        if(in_array($name, $this->tableHasMany))
        {
            $foreignKey = sprintf('%s_id', String::singular($this->tableName));
        ...
        */
        ...
        if($this->tableHasMany)
        {
            foreach($this->tableHasMany as $refModel)
            {
        ...
        /*
        if(in_array($name, $this->tableBelongsTo))
        {
            $table = sprintf('%s_%s', strtolower($module), String::plural($name));
            $foreignKey = $name . '_id';

            $model = new $class();
            return $model->select($table . '.*')
                ->leftJoin($this->tableName, $table . '.id', '=', $this->tableName . '.' . $foreignKey)
                ->where($this->tableName . '.id', '=', $this->id);
        }
        */

        // Check if has tableBelongsTo, if it does select via left join to reference table
        if($this->tableBelongsTo)
        {
            foreach($this->tableBelongsTo as $refModel)
            {
        ...
        // If tableHasAndBelongsToMany (uses seperate reference table
        /*
        if(in_array($name, $this->tableHasAndBelongsToMany))
        {
            $bits = explode('_', $this->tableName);
            $plural = String::plural($name);

            $table = sprintf('%s_%s', strtolower($module), $plural); // blog_posts
            $names = array($bits[1], $plural);
            sort($names);
            $refTable = implode('_', $names); // categories_posts

            $fk1 = $name . '_id'; // Foreign key 1: post_id
            $fk2 = String::singular($bits[1]) . '_id'; // Foreign key 2: category_id

            $model = new $class();
            return $model->select($table . '.*')
                ->leftJoin($refTable, $refTable . '.' . $fk1, '=', $table . '.id')
                ->where($refTable . '.' . $fk2, '=', $this->id);
        }
        */

        if($this->tableHasAndBelongsToMany)
        {
            foreach($this->tableHasAndBelongsToMany as $refModel)
            {
                $refBits = explode('.', $refModel);
    ...
    public function save()
    {
        ...
        // Check if tableHasAndBelongsToMany, running update if necessary
        if($this->tableHasAndBelongsToMany)
        {
            foreach($this->tableHasAndBelongsToMany as $refModel)
            {
    ...
    public function delete($id = null)
    {
        ...
        {
            // Check if tableHasMany (ex: Blog posts have many comments, so delete comments associated with post)  
            if($this->tableHasMany)
            {
                foreach($this->tableHasMany as $refModel)
                {
            ...
            // Check if tableHasAndBelongsToMany
            if($this->tableHasAndBelongsToMany)
            {
